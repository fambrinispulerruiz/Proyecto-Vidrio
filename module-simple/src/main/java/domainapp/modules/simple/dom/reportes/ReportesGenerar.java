...
import domainapp.modules.simple.dom.ordenes_trabajo.OrdenDeTrabajoServices;
...
public class ReportesGenerar {
    ...
    public Blob generarReporteEmpresas() throws JRException, IOException{
    ...
    @Action(semantics = SemanticsOf.SAFE, restrictTo = RestrictTo.PROTOTYPING)
    @ActionLayout(bookmarking = BookmarkPolicy.AS_ROOT, named = "Exportar Ordenes De Trabajo PDF", sequence = "1")
    public Blob generarReporteOrdenesDeTrabajo() throws JRException, IOException{
        return ordenesdetrabajo.generarReporteOrdenDeTrabajo();
    }
    
    @Action(semantics = SemanticsOf.SAFE, restrictTo = RestrictTo.PROTOTYPING)
    @ActionLayout(bookmarking = BookmarkPolicy.AS_ROOT, named = "Exportar Vidrios PDF", sequence = "1")
    public Blob generarReporteVidrios() throws JRException, IOException{
        return vidrios.generarReporteVidrio();
    }

    @Inject RepositoryService repositoryService;
    @Inject EmpresaServices empresas;
    @Inject ModeloServices modelos;
    @Inject OrdenDeTrabajoServices ordenesdetrabajo;
